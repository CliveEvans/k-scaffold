 ...
 */
const build = async ({source ='./',destination,dynamicDestination,testDestination,pugOptions={suppressStack:true},scssOptions={},watch=false,templates,sfc,projectName}) => {
  if (watch) {
    // Activate the dashboard BEFORE the initial build so that any errors
    // from the first processSheet pass are captured by the UI rather than
    // printed to the raw terminal and scrolled above the dashboard.
    reporter.setMode('dashboard');
    reporter.setProjectName(projectName || path.basename(process.cwd()));
    reporter.render(); // draw the initial empty layout immediately
  }
  ...
  if(watch){
    return watchSheet({source,destination,dynamicDestination,testDestination,pugOptions,scssOptions,templates,sfc,projectName});
  }else{
