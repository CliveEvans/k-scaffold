 ...
 * Debounced so that rapid or duplicate events (poll + stdin + stdout +
 * SIGWINCH may all fire within the same tick) coalesce into one redraw.
 *
...
const _onResize = () => {
  ...
  }, 32); // ~2 frames — long enough for all signals to settle
};
 ...
 * Polling is the only approach guaranteed to work there.
 */
const _pollResize = () => {
 ...
 *
 * We listen on stdout, stdin (more reliable on bare Windows consoles where
 * resize events arrive on the input handle), and SIGWINCH on non-Windows, then
 * back all of them up with a 250 ms size poll for environments (VS Code, some
 * shell wrappers) where the event never fires at all.
 */
