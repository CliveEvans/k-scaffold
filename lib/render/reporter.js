 ...
 * Debounced so that rapid or duplicate events (stdin + stdout + SIGWINCH may
 * all fire within the same tick) coalesce into a single redraw.
 *
 * We call logUpdate.clear() before redrawing: a dimension change shifts how
 * many terminal rows the previous output occupied, so log-update's stored line
 * count would be stale without the clear, leaving ghost lines on screen.
 */
...
const _onResize = () => {
  ...
  }, 32); // ~2 frames — enough to let all resize signals settle
};

/**
 * Attach all available resize signal sources in dashboard mode.
 *
 * On Windows the resize event arrives on the console *input* handle
 * (process.stdin / ReadStream), not the output handle; relying only on
 * process.stdout misses vertical resize events there.  Listening on all
 * three sources (stdin, stdout, SIGWINCH on non-Windows) with a debounce
 * gives reliable coverage across platforms and shell wrappers.
 */
const _attachResizeListeners = () => {
  process.stdout.on('resize', _onResize);
  if (process.stdin.isTTY) {
    process.stdin.on('resize', _onResize);
  }
  if (process.platform !== 'win32') {
    process.on('SIGWINCH', _onResize);
  }
};
...
const _detachResizeListeners = () => {
  ...
  if (process.platform !== 'win32') {
    process.off('SIGWINCH', _onResize);
  }
  if (_resizeTimer) {
